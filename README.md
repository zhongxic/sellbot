...
该接口用于重置通话接听时间。由于加载话术、初始化会话，分词器等操作比较耗时，所以存在客户端提前申请开场的场景，此时通话还没有接听，若以初始化开场白的时间作为接听时间，会导致与接听时长相关的意向标签分析不准确，因此需要重置通话接听时间。
...
此接口用于刷新会话的过期时间。当客户经理介入通话后，客户不再与机器人进行交互，但是此时还不能挂断机器人，否则机器人记录的接听时长与实际接听时长会不一致，导致与接听时长相关的意向标签分析不准确。
...
节点即对话过程中的一个流程节点，节点具有分支，分支控制了流程的走向。
...
分支的语义用于更新会话的统计信息，例如命中了 positive 分支，则会话的肯定回答次数增加、命中了 negative 分支，则会话的否定回答次数增加，命中 spacial 或其它分支则不做处理。
...
文案自动跳转 (responses[0].enableAutoJump)：如果文案开启了自动跳转，则不回复该文案，直接跳转文案 next 指向的节点。
...
话术只能具有一个业务问答节点，不同的问题属于业务问答节点不同的分支。话术必须有开始节点和结束节点。

除业务问答外，其余节点必须要有 enter 分支。主流程节点（除结束节点外）必须有且仅有一个未拒绝（positive）分支。

主流程节点 enter 分支的回复文案即节点的文案，其它分支回复文案必须为空，分支 next 属性指向下一个主流程节点。

节点的匹配顺序（order）：仅针对通用对话节点有效，控制不同通用对话匹配的优先级。

节点的分支匹配优先级（matchOrders）：定义当前节点同时命中多个分支时，优先匹配哪一个分支。流程匹配过程中，分支匹配优先级按照自定义优先级、通用对话、当前节点的其他分支，业务问答的顺序合并而来。
...
通用对话节点有 3 种行为：重复上一句（跳转特殊节点 repeat）、回复指定文案，跳转指定节点。除了预定义的通用对话外，可以按照上述行为自定义通用对话。若自定义通用对话节点的类型包含 end_ 前缀，那么将该节点视为结束。
...
意向规则 = 流程条件 + 意图条件。expression 是流程条件，它是一个 [expr](https://github.com/expr-lang/expr) 表达式，使用 expr 表达式并结合[会话统计信息 Status](./internal/service/process/env.go) 可以定义任意条件。intentionCondition 是意图条件，它由所处节点以及需要命中的关键词组成。

### 话术变量

定义话术具有的变量列表。在申请开场白时，机器人会校验参数传递的变量数目是否与话术变量数目一致。在回复文案文本时，会将文本中的变量替换成参数中传递的变量。
